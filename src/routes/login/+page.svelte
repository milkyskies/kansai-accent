...
<div class="w-limit">
	<Card title="">
		<form class="flex flex-col gap-2">
			...
			{/if}
			<input type="email" placeholder="メールアドレス" class="border bg-slate-50 px-1" bind:value={email} />
			<input type="password" placeholder="パスワード" class="border bg-slate-50 px-1" bind:value={password} />
			<div>{message}</div>
			<button type="button" on:click={login} class="rounded-md bg-slate-200 p-1"> Sign In </button>
			<button type="button" on:click={logout} class="rounded-md bg-slate-200 p-1"> Sign Out </button>
			{#if $session?.user.email == "ryan@websculptors.net"}
				<button type="button" on:click={register} class="rounded-md bg-slate-200 p-1"> Register </button>
			{/if}
		</form>
	</Card>
</div>
